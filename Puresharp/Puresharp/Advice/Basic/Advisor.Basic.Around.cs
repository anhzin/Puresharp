    ...
    static public partial class __Advice
    {
        ...
        static public Advice Around(this Advice.Style.IBasic basic, Func<object, object[], Func<object>, object> advice)
        {
            return new Advice((_Method, _Pointer) =>
            {
                ...
                _body.Emit(OpCodes.Newobj, Runtime<Func<object>>.Type.GetConstructors().Single());
                _body.Emit(OpCodes.Call, advice.Method);
                if (_type == Runtime.Void) { _body.Emit(OpCodes.Pop); }
                else
                {
            ...
            where T : class, IDisposable, new()
        {
            return new Advice((_Method, _Pointer) =>
            {
                ...
                if (_type == Runtime.Void)
                {
                    ...
                    _body.Emit(OpCodes.Ldloc_0);
                    _body.Emit(OpCodes.Callvirt, Runtime<IDisposable>.Method(_Disposable => _Disposable.Dispose()));

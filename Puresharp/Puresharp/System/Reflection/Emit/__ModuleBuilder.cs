...
namespace Puresharp
{
    ...
    static internal class __ModuleBuilder
    {
        ...
        {
            var _type = module.DefineType(string.Concat(Runtime<Type>.Type.Name, Guid.NewGuid().ToString("N")), TypeAttributes.Class | TypeAttributes.Sealed | TypeAttributes.Abstract | TypeAttributes.Public);
        ...
        {
            var _field = module.DefineField(name, Runtime<T>.Type);
        ...
        {
            var _field = module.DefineField(Runtime<T>.Type.Name, Runtime<T>.Type);
            _field.SetValue(null, value);
            return _field;
        }
    }

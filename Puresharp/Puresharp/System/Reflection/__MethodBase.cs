    ...
    static internal class __MethodBase
    {
        static private class Initialization
        {
            static public OpCode[] Single()
            {
                ...
                foreach (var _field in Runtime<OpCodes>.Type.GetFields(BindingFlags.Public | BindingFlags.Static))
                {
        ...
        {
            var fiBytes = Runtime<ILGenerator>.Type.GetField("m_ILStream", BindingFlags.Instance | BindingFlags.NonPublic);
            var fiLength = Runtime<ILGenerator>.Type.GetField("m_length", BindingFlags.Instance | BindingFlags.NonPublic);
        ...
        static public bool Attributed<T>(this MethodBase method)
            where T : Attribute
        {
            return System.Attribute.IsDefined(method, Runtime<T>.Type);
        }

        static public T Attribute<T>(this MethodBase method)
            where T : Attribute
        {
            return System.Attribute.GetCustomAttribute(method, Runtime<T>.Type) as T;
        }

        static public IEnumerable<T> Attributes<T>(this MethodBase method)
            where T : Attribute
        {
            return System.Attribute.GetCustomAttributes(method, Runtime<T>.Type).Cast<T>();
        }

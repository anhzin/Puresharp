...
namespace Puresharp
{
    static internal partial class Closure
    {
        public sealed class Routine
        {
            ...
            {
                var _type = Closure.Routine.m_Module.DefineType(string.Concat(Runtime<Type>.Type.Name, Guid.NewGuid().ToString("N")), TypeAttributes.Class | TypeAttributes.Public, Runtime<object>.Type);
                var _constructor = _type.DefineConstructor(MethodAttributes.Public, CallingConventions.HasThis, signature).GetILGenerator();
                var _method = _type.DefineMethod(Runtime<Routine>.Type.Name, MethodAttributes.Public, CallingConventions.HasThis, Runtime.Void, Type.EmptyTypes).GetILGenerator();
                ...
                else
                {
                    ...
                    _constructor.Emit(OpCodes.Ret);
                    _method.Emit(method, type, signature);
                    _method.Emit(OpCodes.Stfld, _type.DefineField(Runtime<Routine>.Field(_Action => _Action.Value).Name, type, FieldAttributes.Public));
                    _method.Emit(OpCodes.Ret);
                    this.Type = _type.CreateType();
                    this.Constructor = this.Type.GetConstructors(BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly)[0];
                    this.Method = this.Type.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly)[0];
                    this.Value = this.Type.GetField(Runtime<Routine>.Field(_Action => _Action.Value).Name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                }
            }
        }
    }

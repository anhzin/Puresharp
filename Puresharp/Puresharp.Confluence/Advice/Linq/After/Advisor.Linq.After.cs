        ...
        static public Advice After(this Advice.Style.ILinq linq, Func<Expression, IEnumerable<Expression>, Expression> advice)
        {
            return new Advice((_Method, _Pointer, _Boundary) =>
            {
                ...
                if (_Boundary != null)
                {
                    ...
                    return _Boundary.Combine(new Advice.Boundary.Advanced.After.Singleton(_Method, Expression.Lambda<Action<object, object[]>>(_advice, _instance, _arguments).Compile()));
                }
                else
                {

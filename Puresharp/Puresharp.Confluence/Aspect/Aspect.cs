...
namespace Puresharp.Confluence
{
    ...
    abstract public partial class Aspect
    {
        ...
        //    var _name = Metadata<Aspect>.Type.Assembly.GetName().Name;
        //    var _version = Metadata<Aspect>.Type.Assembly.GetName().Version.Major.ToString();
        //    return _domain.Where(_Assembly => _Assembly.GetCustomAttributes<AssemblyMetadataAttribute>().Any(_Metadata => _Metadata.Key == _name && _Metadata.Value == _version)).SelectMany(Aspect.Explore);
        //}
    ...
    public sealed class Aspect<T> : Aspect
        where T : class, Advice.IBoundary, new()
    {
        override public IEnumerable<Advice> Advise(MethodBase method)
        {
